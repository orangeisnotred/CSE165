...
public class hearingPoint : MonoBehaviour
{
        ...
        try
        {
            using (StreamReader sr = new StreamReader("sample_short.txt"))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if (i != lineCount - 1)
                    {
                        GameObject prefab = Resources.Load("Sphere_hearing") as GameObject;
                        ...
                        CheckPoint.name = "Sphere_hearing" + i.ToString();
    ...
    void Update()
    {
		...
		/////////////////////////////// WayFinding one//////////////////////////
		nextCP = GameObject.Find("/Sphere_hearing"+(lineCount-2).ToString());
		...
		if(!Finished)
		{
			if (nextCP == null)
			{
				...
				if(GameObject.Find("/Sphere_hearing0") != null)
				{
		...
		if(Finished && Stop)
		{
		 ...
		 // GameObject ALCamera = GameObject.Find("/ALCamera");
		// AudioListener AuListener = ALCamera.GetComponent<AudioListener>();
		
		// if (d<=130)
		// {
			// AuListener.volume =  (1-d/130);
		// }
		// else
		// {
			// AuListener.volume =  0;
